"""Dreamer class for SAILOR-R2: wraps R2WorldModel + ImagBehavior.

Same interface as SAILOR's ``Dreamer`` so ``SAILORTrainer`` works unchanged.
"""
...
from .imag_behavior import ImagBehavior
from .world_model import R2WorldModel

_to_np = lambda x: x.detach().cpu().numpy()


class Dreamer(nn.Module):
    def __init__(self, obs_space, base_policy, config, logger, dataset, expert_dataset=None):
        super().__init__()
        self._config = config
        self._logger = logger
        ...
        # R2WorldModel (r2dreamer encoder/RSSM + SAILOR discriminator)
        self._wm = R2WorldModel(obs_space=obs_space, step=self._step, config=config)
        self._task_behavior = ImagBehavior(config, self._wm, base_policy=base_policy)

        if config.compile:
            cprint("Compiling R2WorldModel + ImagBehavior", "green")
    ...
    def get_action(self, obs_orig, state):
        ...
        embed = self._wm.encoder(obs_dreamer)
        embed = embed.squeeze(1)

        latent, _ = self._wm.dynamics.obs_step(latent, action, embed, obs_dreamer["is_first"])
        feat = self._wm.dynamics.get_feat(latent)

        action_dict = self._task_behavior.get_action(obs=obs_orig, feat=feat, latent=latent)
    ...
    def _train(self, data, training_step):
        metrics = {}
        data_wm = select_latest_obs(data)
        ...
        metrics.update(
            self._task_behavior._train(start, reward, training_step)[-1]
        )
        return metrics

    def save_checkpoint(self, path):
        torch.save(self.state_dict(), path)
        cprint(f"Saved sailor-r2 checkpoint to {path}", "green")

    def load_checkpoint(self, path):
        self.load_state_dict(torch.load(path, weights_only=True))
        cprint(f"Loaded sailor-r2 checkpoint from {path}", "green")
